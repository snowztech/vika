...
A vikusha assistant is defined by a single YAML file. Drop a different YAML, get a different assistant — same binary.
...
transports:
  ...
  - http:
      bind: 127.0.0.1:7070
      auth_token_env: VIKUSHA_HTTP_TOKEN

# Optional — memory backend. Defaults to file.
memory:
  backend: file # file | sqlite | pgvector
  path: ~/.vikusha/coach/memory
  workspace: ~/.vikusha/coach/workspace # files this agent can access
...
logging:
  ...
  file: ~/.vikusha/coach/logs
```

## Validation

`vikusha run character.yaml` validates on startup:
...
```bash
vikusha run characters/vikusha.yaml characters/coach.yaml characters/koda.yaml
```
